/**
 * Nomic Agent for frontend authentication with OAuth.
 */
async function nomicAgentAuthenticate(apiKey, walletId) {
     ...
     */
    try {
        const response = await redaxios.post("https://localhost:8000/api/auth/login", { api_key: apiKey, wallet_id: walletId }, {
...
async function nomicAgentRefreshToken(refreshToken, walletId) {
     ...
     */
    try {
        const response = await redaxios.post("https://localhost:8000/api/auth/refresh", { refresh_token: refreshToken, wallet_id: walletId });
