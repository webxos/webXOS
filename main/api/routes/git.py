...
async def git_push(request: GitRequest, user_id: str = Depends(verify_token)):
    try:
        ...
        repo = git.Repo.clone_from(config.REPO_URL, repo_dir, branch=request.branch)
        repo.git.add(all=True)
        repo.git.commit(m="Automated commit from WEBXOS MCP Gateway")
        repo.git.push("origin", request.branch, env={"GIT_ASKPASS": config.REPO_TOKEN})
...
async def git_pull(request: GitRequest, user_id: str = Depends(verify_token)):
    try:
        ...
        if os.path.exists(repo_dir):
            shutil.rmtree(repo_dir)
        repo = git.Repo.clone_from(config.REPO_URL, repo_dir, branch=request.branch)
        repo.git.pull("origin", request.branch, env={"GIT_ASKPASS": config.REPO_TOKEN})
