...
async def mcp_status(token: str = Depends(get_octokit_auth)):
    try:
        db = await get_db()
        query = "SELECT vial_id, status, quantum_state, wallet_address, mcp_server FROM vials WHERE active = true"
        ...
        with sqlite3.connect("error_log.db") as conn:
            conn.execute("INSERT INTO vial_logs (vial_id, event_type, event_data, node_id) VALUES (?, ?, ?, ?)",
                        ("inspector", "mcp_status", json.dumps({"vials_count": len(vials), "servers": [v["mcp_server"] for v in vials]}), token.get("node_id", "unknown")))
...
async def mcp_connect(operation: dict, token: str = Depends(get_octokit_auth)):
    try:
        ...
        port = operation.get("port", 6277)
        if not vial_id:
            raise ValueError("Vial ID missing")
        query = "UPDATE vials SET mcp_server = $1, mcp_port = $2 WHERE vial_id = $3 RETURNING mcp_server, mcp_port"
        result = await db.execute(query, server, port, vial_id)
        with sqlite3.connect("error_log.db") as conn:
            conn.execute("INSERT INTO vial_logs (vial_id, event_type, event_data, node_id) VALUES (?, ?, ?, ?)",
                        (vial_id, "mcp_connect", json.dumps({"server": server, "port": port}), token.get("node_id", "unknown")))
