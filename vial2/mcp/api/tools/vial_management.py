...
async def handle_command(command: str, request, db):
    try:
        parts = command.split()
        cmd = parts[0].lower()
        if cmd == '/prompt':
            ...
            async with db:
                await db.execute(
                    ...
                    "running" if action.lower().includes("train") else "stopped",
                    action,
                    len(action),
                    vial_id
                )
            ...
            return {"vials": vials, "computes": computes}
        else:
...
async def troubleshoot(db):
    try:
        async with db:
            vials = await db.fetch("SELECT * FROM vials")
            computes = await db.fetch("SELECT * FROM computes")
            logs = await db.fetch("SELECT * FROM logs ORDER BY timestamp DESC LIMIT 10")
        return {"vials": vials, "computes": computes, "logs": logs}
    except Exception as e:
        logger.error(f"Troubleshoot failed: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))

# xAI Artifact Tags: #vial2 #vial_management #pytorch #neon_mcp
