import os
from langchain.adapters.openai import OpenAIAdapter
...
class AgentManager:
    ...
    async def register_agent(self, agent_type: str, credentials: Dict = None):
        try:
            ...
            self.langchain_adapters[agent_type] = OpenAIAdapter(api_key=credentials["api_key"])
            logger.info(f"Registered agent {agent_type} with LangChain adapter")
    ...
    async def get_agent(self, agent_type: str):
        ...
        if agent and agent.get("status") == "active":
            return self.langchain_adapters.get(agent_type)
        raise ValueError(f"Agent {agent_type} not available")

agent_manager = AgentManager()

# xAI Artifact Tags: #vial2 #mcp #tools #agent #manager #langchain #neon_mcp
