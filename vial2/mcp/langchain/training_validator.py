from langchain.trainers import QuantumTrainer
from ..ds import DataSynthesizer
from ..database.neon_connection import neon_db
from ..error_logging.error_log import error_logger
import logging
import git

logger = logging.getLogger(__name__)

class TrainingValidator:
    def __init__(self):
        self.trainer = QuantumTrainer()
        self.ds = DataSynthesizer()
        self.repo = git.Repo(os.getenv("GIT_REPO_PATH", "."))

    async def validate_training(self, vial_id: str, commands: list):
        try:
            prompts = self.ds.generate_git_prompts(commands)
            is_valid = await self.trainer.validate(prompts)
            query = "INSERT INTO vial_logs (vial_id, event_type, event_data) VALUES ($1, $2, $3)"
            await neon_db.execute(query, vial_id, "training_validation", {"valid": is_valid})
            self.repo.index.add(["*"])
            self.repo.index.commit(f"Validated training for vial {vial_id} with result {is_valid}")
            logger.info(f"Validated training for vial {vial_id} with LangChain")
            return is_valid
        except Exception as e:
            error_logger.log_error("training_validate", str(e), str(e.__traceback__), sql_statement=query, sql_error_code=None, params={vial_id})
            logger.error(f"Training validation failed: {str(e)}")
            raise

training_validator = TrainingValidator()

# xAI Artifact Tags: #vial2 #mcp #langchain #training #validator #git #neon_mcp
