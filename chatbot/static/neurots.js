...
const MAX_COLLAB_DISTANCE = 120; // Longer for morphcollab
...
function initDots() {
    dots = [];
    if (isGalaxyMode) {
        for (let i = 0; i < DOT_COUNT * 2; i++) { // More dots for galaxy
    ...
    for (let i = 0; i < dots.length; i++) {
        for (let j = i + 1; j < dots.length; j++) {
            ...
            if (isMorphCollabMode && dots[i].agent !== dots[j].agent) {
                ...
                // Same-agent or default connections
                const dx = dots[i].x - dots[j].x;
                const dy = dots[i].y - dots[j].y;
                const distance = Math.sqrt(dx * dx + dy * dy);
                if (distance < maxDistance) {
...
function updateDots() {
    ...
    if (animationPhase === 'dissipate') {
        ...
        dots.forEach(dot => {
            ...
            if (isGalaxyMode) {
                if (dot.type === 'star') {
                    dot.opacity = 0.5 + Math.sin(Date.now() * 0.005 + dot.index) * 0.5; // Twinkling
        ...
        if (isRandomMode) {
            ...
            growthFactor += 0.01; // Faster growth
        } else if (isGalaxyMode) {
            growthFactor += 0.007; // Moderate growth
        }
