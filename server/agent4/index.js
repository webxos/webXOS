...
async function agent4Search(query, siteIndex, useLLM = false) {
    if (useLLM) {
        try {
...
app.post('/search', async (req, res) => {
    ...
    try {
        ...
        const results = await agent4Search(query, siteIndex, useLLM);
        res.json({
            success: true,
...
app.get('/health', (req, res) => {
    res.json({ status: 'healthy', agent: 'Agent4', version: '1.0.0' });
});
...
if (process.env.NODE_ENV !== 'production') {
    app.listen(port, () => {
        console.log(`Agent4 server running on port ${port}`);
    });
}
...
module.exports.handler = async (event, context) => {
    ...
    if (httpMethod === 'POST' && path === '/search') {
        ...
        return {
            statusCode: 200,
            body: JSON.stringify({ status: 'healthy', agent: 'Agent4', version: '1.0.0' })
        };
