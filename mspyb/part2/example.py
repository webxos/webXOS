...
- Initial v3 release with mandatory Auto-Start / Auto-Launch
...
def write_file(path: str, content: str, extra_vars: Optional[Dict[str, str]] = None) -> None:
...
```bash
python3 bootstrap.py          # from the parent directory
```
...
print("Hello from $PROJECT_NAME — MSPYB v$MSPYB_VERSION full-circle bootstrap succeeded")
""")
...
def auto_launch() -> None:
    if NO_LAUNCH or TEMPLATE_VARS.get("AUTO_LAUNCH", "true").lower() != "true":
        log.info("Auto-launch disabled by flag or TEMPLATE_VARS")
        return

    cwd = os.path.join(PROJECT_ROOT, TEMPLATE_VARS.get("LAUNCH_CWD", "."))
    cmd_str = TEMPLATE_VARS.get("LAUNCH_COMMAND", "python -m app.main")
    # Expand any remaining $VARS that might appear in the command
    cmd_str = render(cmd_str)
    cmd = cmd_str.split()

    log.info("Auto-launching on %s: %s (cwd=%s)", ENV["system"], cmd_str, cwd)
    try:
        ...
        try:
            ...
            if proc.returncode not in (0, None):
                log.warning("Launch process exited with code %s", proc.returncode)
        except subprocess.TimeoutExpired:
            log.info("Process is running (detached after grace period)")
    except FileNotFoundError as exc:
        log.error("Auto-launch failed (command not found): %s — recovery: verify LAUNCH_COMMAND and INSTALLS", exc)
    except Exception as exc:
        log.error("Auto-launch failed: %s — recovery: check LAUNCH_COMMAND, paths, and dependencies", exc)

auto_launch()

print(f"MSPYB {MSPYB_VERSION} bootstrap complete — project generated in '{PROJECT_ROOT}'")
if not NO_LAUNCH and TEMPLATE_VARS.get("AUTO_LAUNCH", "true").lower() == "true":
    print("Application launch attempted (see log above for details)")
else:
    print(f"Next steps: cd {PROJECT_ROOT} && {TEMPLATE_VARS.get('LAUNCH_COMMAND', 'python -m app.main')}")
